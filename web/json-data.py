...
if __name__ == '__main__':
    ...
    with req.urlopen(url) as source:
        ...
        json_data = json.loads(source.read().decode())
        ...
        for i, column in enumerate(json_data['meta']['view']['columns']):
        ...
        # columns 8 (Zip Code) and 9 (Population), and we're going t =o build a dictionary with this information
        ...
        empty = [zipcode for zipcode, n in people.items() if n == 0]
        print('{0} zip codes are empty ({1})\n\n'.format(len(empty), '; '.join(sorted(empty))))

        # Write JUST the data back out to a local JSON file.
        with open("data/write_data.json", 'w') as f:
            json.dump(json_data['data'], f)

    print("Reading back in saved data")
    # You can now read the file back in again, if you want
    with open("data/write_data.json", 'r') as f:
        data = json.load(f)

        for datum in data:
            print("Data element: {0}".format(datum))
